...
Рабочий manifest создается командой `make evidence-manifest` после smoke-проверок, `docker-metadata` и отчета Trivy.
...
интеграционный smoke, автопилотную базу, метаданные образа, SARIF, SBOM и evidence manifest.
