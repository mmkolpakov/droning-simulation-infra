...
Release workflow запускается вручную через GitHub Actions или автоматически по тегам `v*`. Он публикует `linux/amd64` образ в GHCR, добавляет OCI labels, BuildKit cache, SBOM и provenance attestations.
